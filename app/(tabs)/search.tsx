...
interface SearchResultItem { page: Page; book: Book; blockId: string; snippet: string; matchIndex: number }
...
export default function SearchScreen() {
  ...
  const openResult = (item: SearchResultItem) => router.push({
    ...
    params: { page: String(item.page.page_number), pageId: item.page.id, blockId: item.blockId, bookPartId: selectedScope?.part.id ?? item.page.book_part_id, highlight: query },
  });
...
const styles = StyleSheet.create({
  ...
  actionRow: { flexDirection: 'row', gap: 6, paddingHorizontal: AppSpacing.lg, marginTop: AppSpacing.md }, selectorButton: { flexGrow: 1, flexBasis: '76%', minWidth: 0, height: 46, flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: AppSpacing.md, backgroundColor: AppColors.surface, borderWidth: 1, borderColor: AppColors.border, borderRadius: 23 }, selectorText: { flex: 1, color: AppColors.text, fontFamily: AppFonts.sansMedium, fontSize: 14 }, submitButton: { width: 84, height: 46, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4, paddingHorizontal: 8, backgroundColor: AppColors.primary, borderRadius: 23 }, submitText: { color: AppColors.white, fontFamily: AppFonts.sansBold, fontSize: 12 },
