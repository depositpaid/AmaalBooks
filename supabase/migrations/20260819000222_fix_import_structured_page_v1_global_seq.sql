...
BEGIN
  ...
  -- The payload's per-page sequenceIndex is NOT used as the global sequence_index
  -- (which has a unique constraint per book). For new nodes, a global sequence is
  -- computed; for existing nodes, the stored value is preserved.
  ...
  LOOP
    ...
    -- Compute next global sequence_index for potential new node
    SELECT COALESCE(MAX(sequence_index), -1) + 1 INTO v_next_seq
    FROM structural_nodes WHERE book_id = p_book_id;
