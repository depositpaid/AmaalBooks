/*
# Atomic structured-page import RPC

This function is intentionally page-scoped. The reader treats a page as
structured as soon as any content_blocks row exists, so page metadata, relevant
structural nodes, and every block must commit together or not at all.

The function never derives printed numbering, structural identifiers, language,
direction, or corrected text. It accepts only source-reviewed values supplied by
the import manifest. It also refuses to overwrite a page that already has
structured blocks; replacing published/imported transcription requires a future
explicit revision workflow.
*/

CREATE OR REPLACE FUNCTION import_structured_page_v1(
  p_book_id uuid,
  p_edition_id uuid,
  ...
  LOOP
    ...
    IF NOT EXISTS (
      ...
      WHERE node.id = v_node_id
        ...
        AND node.parent_id IS NOT DISTINCT FROM v_parent_id
        ...
        AND node.sequence_index = (v_node->>'sequenceIndex')::int
        AND node.start_page_id IS NOT DISTINCT FROM NULLIF(v_node->>'startPageId', '')::uuid
        AND node.end_page_id IS NOT DISTINCT FROM NULLIF(v_node->>'endPageId', '')::uuid
    ) THEN
...
-- Import is an administrative operation. It is not exposed to the anonymous
-- reader role even though the legacy tables currently retain permissive RLS.
REVOKE ALL ON FUNCTION import_structured_page_v1(uuid, uuid, uuid, uuid, jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION import_structured_page_v1(uuid, uuid, uuid, uuid, jsonb) FROM anon;
GRANT EXECUTE ON FUNCTION import_structured_page_v1(uuid, uuid, uuid, uuid, jsonb)
  TO authenticated, service_role;

COMMENT ON FUNCTION import_structured_page_v1(uuid, uuid, uuid, uuid, jsonb) IS
  'Atomically imports one complete, source-reviewed structured page without replacing legacy page content.';